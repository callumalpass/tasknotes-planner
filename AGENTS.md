...
`@mdbase-dev/connect`. Keep collection access contract-scoped and do not import
source code from the TaskNotes application repository. Date, dependency, and
field behavior that belongs to the portable model should remain in
`@tasknotes/model` rather than being reimplemented here.
